...
class Pipeline(object):
    ...
    def extract(self):
        ...
        self.unemployment = pl.read_excel(url_unemployment, read_options={'header_row': 7})
    ...
    def transform(self):
        ...
        self.population = self.population.with_columns(
            pl.col('YEAR').str.slice(-4))  # e.g. POPESTIMATE2010 -> 2010
        ...
        self.unemployment = self.unemployment.with_columns(
            pl.col('Year').str.slice(-4))  # remove prefix i.e. 'Unemployment_rate_XXXX'

    def load(self):
        db = DB()
        # Insert population data
        db.cur.executemany(
            f"INSERT INTO population VALUES ({','.join(['?'] * len(self.population.columns))})",
            self.population.rows()
        )
        # Insert unemployment data
        db.cur.executemany(
            f"INSERT INTO unemployment VALUES ({','.join(['?'] * len(self.unemployment.columns))})",
            self.unemployment.rows()
        )
        db.conn.commit()
